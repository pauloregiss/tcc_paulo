...
for T = 1:1 % Serão realizados 3 treinamentos
    p = ordem;
    % Obter o conjunto de amostras de treinamento
    dados = load('tcc_treinamento.txt');
    ...
    while true   
        for k = 1:qtde_dados_treinamento - p
            % k = 1
            % x1 = [dados(5) dados(4) dados(3) dados(2) dados(1)]
            % x1 = fliplr(dados(1:5)')
            % d1 = dados(6)
            
            % k = 2
            % x2 = [dados(6) dados(5) dados(4) dados(3) dados(2)]
            % x2 = fliplr(dados(2:6)')
            % d2 = dados(7)

            % k = 3
            % x3 = [dados(7) dados(6) dados(5) dados(4) dados(3)]
            % x3 = fliplr(dados(3:7)')
            % d3 = dados(8)

            % k = 95
            % x95 = [dados(99) dados(98) dados(97) dados(96) dados(95)]
            % x95 = fliplr(dados(95:99)')
            % d95 = dados(100)
    ...
    validacao = load('tcc_validacao.txt');
    ...
    legend('Treinamento','Validação','Location','northeast')
    xlabel('Amostras'),ylabel('Saída da rede'),title(strcat('Treinamento T',num2str(T)))

    % dados(101) vai ser calculada a partir de
    % dados(100),dados(99),dados(98),dados(97),dados(96)
    %yv1 = dados(101)
    %xv1 = fliplr(dados(96:100)')

    % dados(102) vai ser calculada a partir de
    % dados(101),dados(100),dados(99),dados(98),dados(97)
    %yv2 = dados(102)
    %xv2 = fliplr(dados(97:101)')

    % dados(120) vai ser calculada a partir de
    % dados(119),dados(118),dados(117),dados(116),dados(115)
    %yv20 = dados(120)
    %xv20 = fliplr(dados(115:119)')
    ...
    figure(2),plot(Y2,'o-k'),hold on,plot(validacao,'*-r'),grid,%axis([0 9 0 1])
    legend('Saída estimada','Saída desejada','Location','southeast')
    xlabel('Amostras'),ylabel('Saída da rede'),title(strcat('Treinamento T',num2str(T)))
    % Salvar o gráfico
    %grafico = gca;
    %exportgraphics(grafico,strcat('T',num2str(T),' - Saídas','.jpg'))
    ...
    %grafico = gca;
    %exportgraphics(grafico,strcat('T',num2str(T),' - EQM','.jpg'))
    
    % Dados de saída
    EQM_final = eqm(ep)
    Epocas = ep
    EA = abs(validacao - Y2'); % Erro Absoluto
    format short g, Comparacao = [Y2' validacao EA]
    figure(4),plot(EA,'-*'),grid,title('Erro relativo')
    WAPE = 100*sum(EA)/sum(validacao) % Erro Percentual Médio Ponderado (WAPE)
    % A métrica WAPE é muito utilizada em previsão de séries temporais
    % Variância do Erro Relativo (Somatório[(Xi-Xméd)^2])/N 
    Variancia = var(EA) % Percentual
    % Salvar os dados finais
    save(strcat('T',num2str(T),' - Dados de saída'),...
        'EQM_final','Epocas','Comparacao','WAPE','Variancia')
    %close all
end